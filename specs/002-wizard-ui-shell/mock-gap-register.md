...
| **Mocked API Seam**       | `RwaCodegenService` (`validate`, `getAvailableModules`, `generateZip`, `generateFileTree`)                                                                                                                                       |
| **User-Visible Fallback** | None — test-only; not used at runtime                                                                                                                                                                                            |
| **Mock Behavior**         | `createTestCodegenService()` returns deterministic hardcoded validation (always valid), a fixed 3-module catalog (supply-limit, max-balance, country-restrict), a synthetic ZIP blob (not a real JSZip archive), and `generateFileTree` with at least `README.md` whose text matches that blob payload. ZIP↔tree byte parity is proven against the real stellar loader, not this double. `failGenerateFileTree: true` throws `CodegenInvalidConfigError`. |
