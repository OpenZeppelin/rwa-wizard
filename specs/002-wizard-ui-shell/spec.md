...
- What happens when a selected compliance module is marked as under-review? The compliance step should display a visible review badge on the module card with a link to the upstream PR. The module remains selectable but users are informed of its status.
- What happens when a compliance module requires configuration (e.g., a supply limit value)? The compliance step should render inline configuration fields within the module card based on the module's `configFields` metadata, using `TextField`/`NumberField` from the design system.
- What happens when a required module config field is left empty? Field-level validation should indicate the missing value, and the draft should not be marked as ready until all required config fields are provided.
...
- **FR-001C**: Any future deployment-stage placeholder MUST be hidden by default in the MVP and may only be exposed through the existing OpenZeppelin feature-flag system implemented via `@openzeppelin/ui-utils` `AppConfigService` and established app-level feature-config overrides. When later implemented, that UI MUST persist deployment intent through `RWAConfig.deployment.target` (preset/custom reference shape) rather than a flat network string.
...
- **FR-013**: The system MUST present available compliance modules using a module-first catalog sourced from the supported generator integration. Each module card shows its name, description, required hooks, review status (stable or under-review with PR link), and inline configuration fields for module-specific parameters. Users select modules by ID; hooks are derived from registry metadata and displayed in a read-only "Hook Wiring Preview" table. The system MAY expose future target-environment choices only when the corresponding deployment placeholder feature flag is enabled, and any such choice must ultimately populate `config.deployment.target`.
...
- **Capability Catalog**: The set of currently supported choices exposed to the wizard. For compliance, this includes the full module registry with `requiredHooks`, review state, and `configFields` metadata, enabling a module-first selection experience where hooks are derived automatically.
