...
- **ComplianceModuleRegistry**: A catalog of available compliance modules with their metadata (name, description, supported hooks). Only lists modules with concrete implementations. Stellar-specific; future generators provide their own registries. Hook values in `supportedHooks` are ecosystem-defined strings (Stellar uses `StellarComplianceHook`).
