...
2. **`@openzeppelin/rwa-config`** — A shared configuration package that owns the canonical `RWAConfig` type, validation schema, and related types (e.g., `ClaimTopic`, `TrustedIssuer`, `OperatorRole`). Chain-agnostic by design — it describes _what_ the user wants, not _how_ it maps to a specific chain. The `ComplianceHook` type is an opaque `string`; each ecosystem defines its valid values (e.g., Stellar has 5 hooks, EVM T-REX has 4). All RWA generators depend on this package for the shared configuration type.
...
3. **Given** a configuration with compliance modules selected, **When** the generator is invoked, **Then** the deploy script deploys each module contract, calls `set_compliance_address` and `set_identity_registry_storage` on it, registers it on the Compliance contract for each of its `requiredHooks` via `add_module_to`, and calls `verify_hook_wiring`.
...
- **SR-013**: The deploy script MUST perform post-deployment configuration in the correct order: bind token on Compliance and IRS; for each compliance module: call `set_compliance_address`, `set_identity_registry_storage`, register the module on all its `requiredHooks` via `add_module_to`, and call `verify_hook_wiring`; add claim topics; add trusted issuers; and optionally mint initial supply.
