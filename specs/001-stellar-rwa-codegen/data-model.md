...
interface TokenConfig {
  name: string; // max 32 UTF-8 bytes
  symbol: string; // max 12 chars
  decimals: number; // 0–18
  initialSupply?: string; // bigint-compatible string, optional, advisory for deferred post-deploy minting
  documentManager: {
    enabled: boolean;
  };
}
...
- `name`: required, 1–32 UTF-8 bytes
- `symbol`: required, 1–12 characters
- `decimals`: required, integer 0–18
- `initialSupply`: optional, must be non-negative if provided; on Stellar it emits a warning and post-deploy guidance because verified-holder bootstrap is out of scope for spec 001
...
- `roles[].symbol`: optional; if provided, max length is chain-specific (e.g., Soroban's `symbol_short!` limit is 9 chars). If omitted, auto-generated by the generator from `name`.
...
type DeploymentTarget =
  | {
      kind: 'preset';
      ecosystem: string;
      networkId: string; // adapter-defined preset, e.g. "stellar-testnet"
    }
  | {
      kind: 'custom';
      ecosystem: string;
      rpcUrl: string; // custom RPC endpoint
      explorerUrl?: string; // optional explorer base URL
...
- `target`: required
- `target.ecosystem`: required, non-empty string
- `target.kind === 'preset'`: `networkId` required
- `target.kind === 'custom'`: `rpcUrl` required; `explorerUrl` optional
- `@openzeppelin/codegen-rwa-stellar` currently accepts only `ecosystem === "stellar"` and validates preset IDs against adapter-backed Stellar networks

**Notes**:

- `DeploymentTarget` is intentionally serializable and adapter-aligned. The shared config stores lightweight references/URLs rather than adapter runtime objects.
- `@openzeppelin/codegen-rwa-stellar` resolves preset `networkId` values internally via `@openzeppelin/adapter-stellar`, keeping `@openzeppelin/rwa-config` headless and chain-agnostic.
