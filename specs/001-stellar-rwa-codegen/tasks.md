...
- [x] T056 [US6] Implement compliance module contract template in `packages/codegen-rwa-stellar/src/templates/contracts/compliance-module.ts`: thin wrapper re-exporting upstream implementation, `__constructor(e: &Env, admin: Address)`, `set_compliance_address`, `set_identity_registry_storage`, `verify_hook_wiring` stubs, review-status banner for under-review modules
...
- [x] T059 [US6] Add compliance module deployment to deploy.sh template — deploy each unique module, call `set_compliance_address`, `set_identity_registry_storage`, register on all `requiredHooks` via `add_module_to`, call `verify_hook_wiring`
