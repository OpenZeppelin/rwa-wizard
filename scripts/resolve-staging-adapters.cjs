 ...
 * tree is already in place. This script only swaps adapter packages.
 *
...
/** Adapters that exist in this monorepo; each maps to workspace package names that declare it. */
const ADAPTER_TARGET_FILTERS = {
  '@openzeppelin/adapter-evm': ['@openzeppelin/rwa-wizard-app'],
  '@openzeppelin/adapter-stellar': ['@openzeppelin/rwa-wizard-app', '@openzeppelin/codegen-rwa-stellar'],
};

const ADAPTER_PACKAGES = Object.keys(ADAPTER_TARGET_FILTERS);
...
function main() {
  ...
  const results = [];
  /** @type {Array<{ pkg: string, version: string }>} */
  const overrides = [];
  ...
  for (const pkg of ADAPTER_PACKAGES) {
    ...
    if (best.source === 'latest') {
      ...
      overrides.push({ pkg, version: best.version });
    }
  }
  ...
  for (const { pkg, version } of overrides) {
    const filters = ADAPTER_TARGET_FILTERS[pkg];
    const addArgs = ['add', `${pkg}@${version}`, '--save-exact', ...filters.flatMap((f) => ['--filter', f])];
  ...
  console.log(`\n✅ Successfully overrode ${overrides.length} adapter package(s).\n`);
}
