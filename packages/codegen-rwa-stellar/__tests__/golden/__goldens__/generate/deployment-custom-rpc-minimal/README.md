...
Deploy all contracts to Custom RPC (https://rpc.example.test):
...
| Insufficient balance errors | Deploy signer unfunded | Fund the deploy signer on the target network |
...
flowchart TD
  ...
  s0["build.sh → stellar contract build"] --> s1["deploy.sh (Custom RPC (https://rpc.example.test))"]
