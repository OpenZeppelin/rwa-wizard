...
stellar contract invoke \
  ...
  -- \
  add_claim_topic \
  --claim_topic 7 --operator "$MANAGER"
echo "${GREEN}  ✓ Claim topic 7 (Accredited Investor)${RST}"
echo ""
echo "${BOLD}───────────────────────────────────────────────────────────────${RST}"
echo "${BOLD}  Trusted Issuers (2)${RST}"
echo "${BOLD}───────────────────────────────────────────────────────────────${RST}"
stellar contract invoke \
  --id $CTI_ADDRESS \
  --source-account "$MANAGER_SOURCE_ACCOUNT" \
...
stellar contract invoke \
  ...
  -- \
  add_trusted_issuer \
  --trusted_issuer "GCEXAMPLEISSUER2" --claim_topics '[7]' --operator "$MANAGER"
echo "${GREEN}  ✓ Issuer GCEXAMPL... → topics [7]${RST}"
