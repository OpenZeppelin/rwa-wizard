...
`build.sh` compiles **8** WASM artifacts: **6** deployed by `deploy.sh`, plus **2** example/dev-only crates (`rwa_claim_issuer_example.wasm`, `rwa_identity_example.wasm`). Preflight validates only the 6 deployable artifacts.
...
# If preflight prints Manager invokes (country restriction),
...
flowchart TD
  ...
  s2 --> s3["Deploy compliance module"]
...
|--------|-------|--------|--------|
| Country Restriction (`country-restrict`) | `transferred`, `created` | None | Stable |
