...
# Admin for this project: GCMULTISIG
...
stellar keys address <your-identity>   # must output GCMULTISIG
...
|------|---------|-------------------------|-------|
| Owner (multi-sig) | `GCMULTISIG` | ADMIN_SOURCE_ACCOUNT (defaults to SOURCE_ACCOUNT) | Contract admin address baked into deploy.sh. |
| Admin | `GCMULTISIG` | ADMIN_SOURCE_ACCOUNT (defaults to SOURCE_ACCOUNT) |  |
...
|---------|--------------|-----|
| `Missing signing key for GCMULTISIG` (or Manager) during post-deploy | `STELLAR_ACCOUNT` identity does not control Admin/Manager | `stellar keys address <identity>` must match the configured role address, or regenerate with your own address |
...
# If preflight prints Manager invokes (compliance limits on the `created` hook),
...
flowchart TD
  ...
  s6 --> s7["Initial supply: bootstrap-demo-mint.sh (testnet demo)"]
