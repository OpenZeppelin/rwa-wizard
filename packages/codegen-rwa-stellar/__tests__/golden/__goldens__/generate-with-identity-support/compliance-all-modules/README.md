...
`build.sh` compiles **14** WASM artifacts: **12** deployed by `deploy.sh`, plus **2** example/dev-only crates (`rwa_claim_issuer_example.wasm`, `rwa_identity_example.wasm`). Preflight validates only the 12 deployable artifacts.
...
| Manager (deploy/post-deploy) | `GCEXAMPLEMGR` | MANAGER_SOURCE_ACCOUNT (defaults to SOURCE_ACCOUNT) | First configured Manager address; post-deploy invokes sign with this role. |
| Agent | `GCEXAMPLEAGNT` | — | Operational role assigned at deploy; configure signers separately for day-two ops. |
...
### Testnet demo auto-mint workflow
...
# If preflight prints Manager invokes (supply limit, max balance, country restriction, and country allow-list),
...
flowchart TD
  ...
  s2 --> s3["Deploy 7 compliance modules"]
  s3 --> s4["Deploy RWA token"]
  s4 --> s5["Post-deploy: bind token on Compliance and IRS"]
  s5 --> s6["Post-deploy: configure modules and register hooks on Compliance"]
  s6 --> s7["Post-deploy: add 2 claim topics on CTI"]
  s7 --> s8["Post-deploy: add trusted issuer on CTI"]
  s8 --> s9["Initial supply: bootstrap-demo-mint.sh (testnet demo)"]
  s9 --> s10["Deployment summary"]
```

If `token.initialSupply` is set, note that `deploy.sh` does **not** auto-mint it. The upstream claim-based identity flow requires a trusted claim issuer contract, a per-holder identity contract with claims, and IRS registration for the mint recipient before `mint` can pass identity verification. This export includes `scripts/bootstrap-demo-mint.sh` (testnet demo only) which deploys the example Claim Issuer and Identity contracts, onboards Admin with demo claims, and mints the configured amount after `./scripts/deploy.sh`. The configured initial supply is expressed in on-chain base units (smallest token units), not display units; with `token.decimals = 18`, one whole token equals `10^18` base units.
...
This export includes upstream **example** Claim Issuer and per-holder Identity contracts under `contracts/claim-issuer` and `contracts/identity`, plus a `tools/sign-claim` helper. `build.sh` compiles them alongside the core contracts, but `deploy.sh` does **not** deploy or wire them automatically — use them for local/testnet onboarding demos after the core system is deployed. When `token.initialSupply` is set on testnet, this export also includes `scripts/bootstrap-demo-mint.sh` — a **demo-only** script that deploys the example contracts, onboards Admin with hardcoded demo claims, and mints the configured initial supply after `./scripts/deploy.sh`. It is educational scaffolding, not production KYC.
...
| Module | Hooks | Config | Review |
|--------|-------|--------|--------|
| Supply Limit (`supply-limit`) | `created`, `destroyed` | `limit=1000000` | Stable |
| Max Balance (`max-balance`) | `transferred`, `created`, `destroyed` | `maxBalance=50000` | Stable |
| Country Restriction (`country-restrict`) | `transferred`, `created` | `restrictedCountries=US` | Stable |
| Country Allow-list (`country-allow`) | `transferred`, `created` | `allowedCountries=CH, SG` | Stable |
| Transfer Allow-list (`transfer-allow`) | `transferred` | `allowedUsers=GCEXAMPLEOWNER` | Stable |
| Initial Lockup Period (`initial-lockup-period`) | `transferred`, `created`, `destroyed` | `lockupPeriodLedgers=17280` | Stable |
| Time-based Transfer Limits (`time-transfers-limits`) | `transferred` | `limitDurationLedgers=17280`, `limitValue=25000` | Stable |
