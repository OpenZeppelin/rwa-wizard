...
MODULE_SUPPLY_LIMIT_ADDRESS="$(load_manifest_field MODULE_SUPPLY_LIMIT_ADDRESS)"
if [ -z "$MODULE_SUPPLY_LIMIT_ADDRESS" ]; then
  echo "deployment-manifest.json is missing contracts.MODULE_SUPPLY_LIMIT_ADDRESS"
  exit 1
fi
MODULE_MAX_BALANCE_ADDRESS="$(load_manifest_field MODULE_MAX_BALANCE_ADDRESS)"
if [ -z "$MODULE_MAX_BALANCE_ADDRESS" ]; then
  echo "deployment-manifest.json is missing contracts.MODULE_MAX_BALANCE_ADDRESS"
  exit 1
fi
MODULE_COUNTRY_RESTRICT_ADDRESS="$(load_manifest_field MODULE_COUNTRY_RESTRICT_ADDRESS)"
if [ -z "$MODULE_COUNTRY_RESTRICT_ADDRESS" ]; then
  echo "deployment-manifest.json is missing contracts.MODULE_COUNTRY_RESTRICT_ADDRESS"
  exit 1
fi
MODULE_COUNTRY_ALLOW_ADDRESS="$(load_manifest_field MODULE_COUNTRY_ALLOW_ADDRESS)"
if [ -z "$MODULE_COUNTRY_ALLOW_ADDRESS" ]; then
  echo "deployment-manifest.json is missing contracts.MODULE_COUNTRY_ALLOW_ADDRESS"
  exit 1
fi
MODULE_TRANSFER_ALLOW_ADDRESS="$(load_manifest_field MODULE_TRANSFER_ALLOW_ADDRESS)"
if [ -z "$MODULE_TRANSFER_ALLOW_ADDRESS" ]; then
  echo "deployment-manifest.json is missing contracts.MODULE_TRANSFER_ALLOW_ADDRESS"
  exit 1
fi
MODULE_INITIAL_LOCKUP_PERIOD_ADDRESS="$(load_manifest_field MODULE_INITIAL_LOCKUP_PERIOD_ADDRESS)"
if [ -z "$MODULE_INITIAL_LOCKUP_PERIOD_ADDRESS" ]; then
  echo "deployment-manifest.json is missing contracts.MODULE_INITIAL_LOCKUP_PERIOD_ADDRESS"
  exit 1
fi
MODULE_TIME_TRANSFERS_LIMITS_ADDRESS="$(load_manifest_field MODULE_TIME_TRANSFERS_LIMITS_ADDRESS)"
if [ -z "$MODULE_TIME_TRANSFERS_LIMITS_ADDRESS" ]; then
  echo "deployment-manifest.json is missing contracts.MODULE_TIME_TRANSFERS_LIMITS_ADDRESS"
  exit 1
fi
...
echo ""
echo "${BOLD}  Reading on-chain supply limit...${RST}"
ON_CHAIN_SUPPLY_LIMIT=$(stellar contract invoke \
  ...
  -- \
  get_supply_limit \
  --token "$RWA_TOKEN_ADDRESS" 2>/dev/null | grep -Eo '[0-9]+' | tail -1 || true)
echo "    Wizard config at generation: 1000000 · Mint amount: $INITIAL_SUPPLY base units"
echo "    On-chain value now: $ON_CHAIN_SUPPLY_LIMIT (compare before minting)"
echo ""
echo "${BOLD}  Reading on-chain max balance...${RST}"
ON_CHAIN_MAX_BALANCE=$(stellar contract invoke \
  ...
  get_max_balance \
  --token "$RWA_TOKEN_ADDRESS" 2>/dev/null | grep -Eo '[0-9]+' | tail -1 || true)
echo "    Wizard config at generation: 50000 · Mint amount: $INITIAL_SUPPLY base units"
echo "    On-chain value now: $ON_CHAIN_MAX_BALANCE (compare before minting)"
echo ""
echo "${BOLD}  Supply Limit${RST}"
echo "    Supply Limit is 1000000 base units but initialSupply is 1000000000000000000000000. The \`created\` hook runs before mint succeeds — total supply after mint would exceed the configured cap."
echo "    Run this Manager invoke manually, then re-run this script:"
  echo "      stellar contract invoke \\"
  echo "        --id \$MODULE_SUPPLY_LIMIT_ADDRESS \\"
  echo "        --source-account \"\$MANAGER_SOURCE_ACCOUNT\" \\"
  echo "        --network testnet \\"
  echo "        -- \\"
  echo "        set_supply_limit \\"
  echo "        --token \"\$RWA_TOKEN_ADDRESS\" --limit 1000000000000000000000000 --operator \"\$MANAGER\""
  failed=1
echo ""
echo "${BOLD}  Max Balance${RST}"
echo "    Max Balance is 50000 base units but initialSupply is 1000000000000000000000000. The \`created\` hook caps each recipient's balance — Admin cannot receive the full mint amount. Plain language: this is a per-wallet holding limit, not an investor-only rule; Admin and treasury mints are checked the same way. Raise the cap, mint in tranches, or split across wallets."
echo "    Run this Manager invoke manually, then re-run this script:"
  echo "      stellar contract invoke \\"
  echo "        --id \$MODULE_MAX_BALANCE_ADDRESS \\"
  echo "        --source-account \"\$MANAGER_SOURCE_ACCOUNT\" \\"
  echo "        --network testnet \\"
  echo "        -- \\"
  echo "        set_max_balance \\"
  echo "        --token \"\$RWA_TOKEN_ADDRESS\" --max 1000000000000000000000000 --operator \"\$MANAGER\""
  failed=1
echo ""
  if [ "$failed" -ne 0 ]; then
echo "${YELLOW}  Compliance preflight failed — mint would revert on \`created\`.${RST}"
echo "  Fix limits in the wizard and regenerate, or run the Manager invokes printed above, then re-run this script."
    return 1
  fi
echo "${GREEN}  ✓ Compliance preflight passed for demo mint.${RST}"
  return 0
}
