...
stellar keys address <your-identity>   # must output GCDAO
...
|------|---------|-------------------------|-------|
| Owner (dao) | `GCDAO` | ADMIN_SOURCE_ACCOUNT (defaults to SOURCE_ACCOUNT) | Contract admin address baked into deploy.sh. |
| Admin | `GCDAO` | ADMIN_SOURCE_ACCOUNT (defaults to SOURCE_ACCOUNT) |  |
...
|---------|--------------|-----|
| `Missing signing key for GCDAO` (or Manager) during post-deploy | `STELLAR_ACCOUNT` identity does not control Admin/Manager | `stellar keys address <identity>` must match the configured role address, or regenerate with your own address |
